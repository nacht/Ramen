...
CompBuf *alloc_compbuf(int sizex, int sizey, int type, int alloc)
{
    ...
    if( alloc)
    {
        ...
        {
        case CB_RGBA:
            cbuf->rect = MEM_callocN( 4*sizeof(float)*sizex*sizey, "compbuf RGBA rect");
        break;

        case CB_VEC3:
            cbuf->rect = MEM_callocN( 3*sizeof(float)*sizex*sizey, "compbuf Vector3 rect");
        break;

        case CB_VEC2:
            cbuf->rect = MEM_callocN( 2*sizeof(float)*sizex*sizey, "compbuf Vector2 rect");
        break;

        case CB_VAL:
            cbuf->rect = MEM_callocN( sizeof( float)*sizex*sizey, "compbuf Fac rect");
        break;
        }
