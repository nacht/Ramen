...
public:
    ...
    */
    ...
    \note This constructor is for value types which do not support the move library.
    */
    template <typename U>
    ...
    {
        /*
            NOTE : This should not move to the initializer; in case the
            allocation or the move should fail (throw) we want to keep
            the copy_on_write instance in a valid state.
        */
        object_m = allocate_move(0, move(x));
    }
    ...
    \note This constructor is for value types which do support the move library.
    */
    template <typename U>
        ...
        object_m(0)
    {
        /*
            ...
            the copy_on_write instance in a valid state.
        */
        object_m = allocate(static_cast<implementation_t*>(0), x);
    }
    ...
    */
    ...
    template<typename U>
    typename move_sink<U, T, copy_on_write&>::type operator=(U x)
    {
        if (!object_m)
            object_m = allocate_move(0, move(x));
        else if (object_m->header_m.get().count_m.is_one())
            object_m->value_m = move(x);
        else
            reset(allocate_move(object_m, move(x)));

        return *this;
    }

    /*
        Doxygen has a bug where documentation is used only from the last item in
        the group.
    */
    /*!
        Assign a value which is convertible to value_type directly to a
        copy_on_write object.
    */
    template<typename U>
    typename copy_sink<U, T, copy_on_write&>::type operator=(const U& x)
    {
        if (!object_m)
            object_m = allocate(0, x);
        else if (object_m->header_m.get().count_m.is_one())
            object_m->value_m = x;
        else
            reset(allocate(object_m, x));
...
private:
    ...
    template <typename U>
    static implementation_t* allocate_move(const implementation_t* alloc_src, U x)
    {
        implementation_allocator_type allocator(alloc_src ?
                                                alloc_src->get_allocator() :
                                                implementation_allocator_type());
        ...
        try {
            ::new(static_cast<void*>(tmp)) implementation_t(move_from<U>(x));
        } catch (...) {
            tmp->get_allocator().deallocate(tmp, 1);
            throw;
        }
...
struct copy_on_write<T, A>::implementation_t : private boost::noncopyable
{
    ...
    template <typename U>
    explicit implementation_t(const U& x, typename copy_sink<U, T>::type = 0) :
        value_m(x)
    { }

    template <typename U>
    explicit implementation_t(U x, typename move_sink<U, T>::type = 0) :
        value_m(move(x))
    { }
