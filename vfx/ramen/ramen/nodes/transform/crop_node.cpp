...
#include<ramen/nodes/transform/crop_node.hpp>
...
void crop_node_t::do_calc_bounds( const render::render_context_t& context)
{
    Imath::Box2i bounds( intersect( input()->bounds(), domain()));
    set_bounds( bounds);
}

void crop_node_t::do_calc_inputs_interest( const render::render_context_t& context)
{
...
void crop_node_t::serialize( boost::archive::polymorphic_iarchive& ar, const unsigned int version)
{
    ar & BOOST_SERIALIZATION_BASE_OBJECT_NVP(image_node_t);
}

void crop_node_t::serialize( boost::archive::polymorphic_oarchive& ar, const unsigned int version) const
{
    ar & BOOST_SERIALIZATION_BASE_OBJECT_NVP(image_node_t);
}
...
node_info_t crop_node_info()
{
    node_info_t info;
    info.id = "builtin.crop";
...
static bool registered = node_factory_t::Instance().register_node(crop_node_info());

} // namespace

BOOST_CLASS_EXPORT( ramen::crop_node_t)
